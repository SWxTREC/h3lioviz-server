...
|Process the data by calling `process_output.py` with the path to the directory which contains all the data files. Note that if you are processing SWPC data the final directory must match the following regex for run_id to be correctly as run_id is identified from the path. Otherwise a hash of the metadata will be used for the run_id. Regex for SWPC `^.*wsa_enlil_\d{5}\.\d*\.dbqs0`. Note that the downsampling has already been configured (8x radius, 2x latitude, and 2x longitude downsample) and you do not need to pass any flags. If you would like to create custom downsamples, you can run `python3 process_output.py -h` to find the appropriate flags.|`python3 process_output.py <path_to_nc_files>`|`python3 process_output.py ~/wsa_enlil_57671.77346046.dbqs01/`|
