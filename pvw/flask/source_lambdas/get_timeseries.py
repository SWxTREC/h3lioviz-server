...
def lambda_handler(event, context):
    # Get the run folder from the endpoint id that was hit
    # url should be /getTimeSeries/{id}/{satellite}
    runid = event["pathParameters"]["id"]
    satellite = event["pathParameters"]["satellite"]
    # In case the frontend passes .jsond in, turn it into a trailing .json
    satellite = satellite.rstrip("d")
    key = f"data/h3lioviz/pv-ready-data-{runid}/evo.{satellite}"
